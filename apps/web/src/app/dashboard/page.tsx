...
import { useAgents, useExecutions, useExecutionMetrics, useTriggers } from "../../lib/react-query";
import { useAuth } from "../../lib/auth-guard";
import { LoadingGrid, LoadingCard, LoadingSkeleton } from "../../components/loading/LoadingOverlay";
import { usePageLoading } from "../../lib/loading-hooks";

export default function DashboardPage() {
  const { user } = useAuth();
  const { startPageLoad, stopPageLoad, isPageLoading } = usePageLoading('dashboard');

  // Fetch real data
  const { data: agents, isLoading: agentsLoading } = useAgents();
  const { data: executionsData, isLoading: executionsLoading } = useExecutions({ limit: 4 });
  const { data: metrics, isLoading: metricsLoading } = useExecutionMetrics();
  const { data: triggers, isLoading: triggersLoading } = useTriggers();

  const isLoading = agentsLoading || executionsLoading || metricsLoading || triggersLoading;
          ...
          <Card className="bg-white/5 border-white/10 hover:bg-white/10 transition-all duration-300">
            ...
            <CardContent>
              {isLoading ? (
                ...
                <>
                  <div className="text-2xl font-bold text-white">{metrics?.totalExecutions?.toLocaleString() || 0}</div>
                  <p className="text-xs text-gray-400 flex items-center mt-1">
                    <Activity className="h-3 w-3 mr-1" />
                    {executionsData?.executions?.filter(e => e.status === 'running').length || 0} running
                  </p>
                </>
              )}
            </CardContent>
          ...
          <Card className="bg-white/5 border-white/10 hover:bg-white/10 transition-all duration-300">
            ...
            <CardContent>
              {isLoading ? (
                ...
                <>
                  <div className="text-2xl font-bold text-white">{metrics?.successRate?.toFixed(1) || 0}%</div>
          ...
          <Card className="bg-white/5 border-white/10 hover:bg-white/10 transition-all duration-300">
            ...
            <CardContent>
              {isLoading ? (
                ...
                <>
                  <div className="text-2xl font-bold text-white">{triggers?.filter(t => t.isActive).length || 0}</div>
                  <p className="text-xs text-gray-400 flex items-center mt-1">
              ...
              {isLoading ? (
                ...
                <>
                  {agents.slice(0, 4).map((agent) => (
                    <div key={agent.id} className="flex items-center justify-between p-3 bg-white/5 rounded-lg border border-white/10">
              ...
              {isLoading ? (
                ...
                <>
                  {executionsData.executions.slice(0, 4).map((execution) => (
                    <div key={execution.id} className="flex items-center justify-between p-3 bg-white/5 rounded-lg border border-white/10">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <p className="text-sm font-medium text-white">{execution.agentName}</p>
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-xs text-gray-500">Create an agent to see executions here</p>
                </div>
              )}
            </CardContent>
          </Card>
        ...
        <Card className="bg-white/5 border-white/10">
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <Link href="/dashboard/agents/new">
                <Button variant="outline" className="h-20 flex-col border-white/20 text-white hover:bg-white/10 w-full">
